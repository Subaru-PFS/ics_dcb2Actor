...
local function getOutlet(key)
    ...
    for i = 1, #names do
       if names[i]==key then return outlets.loutlets[i]
       end
    end
    return false
end
...
local function stateToBool(state)
   if state=='on' then return true
   elseif state=='off' then return false
   else error(string.format('unknown state : %s', state))
   end
end

local function boolToState(bool)
   if bool==true then return 'on'
   elseif bool==false then return 'off'
   else error(string.format('unknown bool : %s', bool))
   end
end
